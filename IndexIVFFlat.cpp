    ...
    if (metric_type == METRIC_INNER_PRODUCT) {
        if (store_pairs) {
            return new IVFFlatScanner<
                METRIC_INNER_PRODUCT, true, CMin<float, idx_t> > (d);
        } else {
            return new IVFFlatScanner<
                METRIC_INNER_PRODUCT, false, CMin<float, idx_t> >(d);
        }
    } else if (metric_type == METRIC_L2) {
        if (store_pairs) {
            return new IVFFlatScanner<
                METRIC_L2, true, CMax<float, idx_t> > (d);
        } else {
            return new IVFFlatScanner<
                METRIC_L2, false, CMax<float, idx_t> >(d);
        }
    }
    return nullptr;
}
